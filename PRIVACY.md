...
- **Secure Transmission:** Data is signed using HMAC-SHA512 during transmission to ensure integrity.
- **Geolocked Storage:** All collected data is securely stored in **Alibaba Cloud Object Storage Service (OSS)**.
    - **Region:** The backend code explicitly hardcodes the region to **`oss-cn-shanghai` (Shanghai, China)**. This ensures your data remains within the defined geographical boundary.
...
- **Source Code:** Our backend logic (including Netlify Functions and Edge Functions) is open-sourced under the **AGPL-3.0** license. You can review the code at any time on GitHub:
    - Repository: [teddyxlandlee/langpatch-telemetry](https://github.com/teddyxlandlee/langpatch-telemetry)
- **Data Relay:** Due to technical constraints (Edge Functions cannot directly run the Alibaba Cloud SDK), your data is first sent to **Netlify Functions** for relaying.
    - **No-Log Policy:** Netlify Functions act solely as a "pipe" to invoke the Alibaba Cloud SDK and write data to OSS. **The relay server does not store, log, or analyze any of your data**; data is discarded immediately after processing.

### 6. Third-Party Services

- **CDN Services:** This software relies on jsDelivr CDN (https://www.jsdelivr.com/) to load certain dependencies (such as `ali-oss` and `uuidv7`). The privacy practices of these service providers are subject to their respective terms.
...
- **加密传输**：数据在传输过程中使用 HMAC-SHA512 进行签名，确保数据完整性。
- **地域锁定存储**：所有收集到的数据将安全地存储在 **阿里云对象存储（Aliyun OSS）** 服务中。
    - **区域**：后端代码中已硬编码指定了区域为 **`oss-cn-shanghai`（中国上海）**。这确保了您的数据物理上仅存储于该地理边界内。
...
- **开源代码库**：我们的后端处理逻辑（包括 Netlify Functions 和 Edge Functions）均采用 **AGPL-3.0** 协议开源。您可以随时在 GitHub 上审查代码：
    - 仓库地址：[https://github.com/teddyxlandlee/langpatch-telemetry](https://github.com/teddyxlandlee/langpatch-telemetry)
- **数据中转**：由于技术架构限制（Edge Functions 无法直接运行阿里云 SDK），您的数据会先发送至 **Netlify Functions** 进行中转。
    - **不落地原则**：Netlify Functions 仅作为“管道”，负责调用阿里云 SDK 将数据写入 OSS。**该中转服务器不存储、不记录、不分析任何您的数据**，数据在处理完成后即刻丢弃。

### 6. 第三方服务

- **CDN 服务**：本软件依赖 jsDelivr CDN (https://www.jsdelivr.com/) 来加载部分依赖库（如 `ali-oss` 和 `uuidv7`）。这些服务提供商的隐私政策受其各自条款约束。
